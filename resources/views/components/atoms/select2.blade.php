...
@if ($source != '')
    <select name="{{ $attributes->get('name') . ($attributes->has('multiple') ? '[]' : '') }}" data-plugin="select-2"
        @if ($attributes->has('multiple')) multiple="true" @endif data-source="{{ $source }}"
...
@else
    <select name="{{ $attributes->get('name') . ($attributes->has('multiple') ? '[]' : '') }}" data-plugin="select-2"
        @if ($attributes->has('multiple')) multiple="true" @endif data-placeholder="{{ $placeholder }}"
        data-allow-clear="{{ $allowClear }}" data-dropdown-parent="{{ $parent }}"
        {{ $attributes->merge(['class' => 'form-select ']) }}>
        <option value="" selected disabled></option>
        {{ $slot }}
        @if (count($lists) > 0)
